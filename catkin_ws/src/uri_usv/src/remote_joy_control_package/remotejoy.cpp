/**
 *
 * Modified from motorcontrol for remote joy control from upper computer.
This program need heartbeat to keep running
 */
...
int frequency = 50,period,heartbeat=0,count=0;  // set the frequency of the pwm
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber sub5 = motor_joy.subscribe("/heartbeat", 2, heartbeatCallback);
  while (ros::ok())
  {
	  count++;
	 	  if(count>200){
