...
int main(int argc, char** argv)
{
  ...
  if(chargeFlag==0){
	  pcsin.open("/home/ubuntu/catkin_ws/src/asv_control/files/parameters.txt");
	  pcsin>>batt_state.charge;
	  pcsin.close();
  }
  pcsout.open("/home/ubuntu/catkin_ws/src/asv_control/files/parameters.txt",ios::trunc);
  signal(SIGINT, MySigintHandler);
  // create a Publisher and publish a string topic named heading
  ros::Publisher batt_pub = nh.advertise<sensor_msgs::BatteryState>("/sensor/VI/m_power", 2);
