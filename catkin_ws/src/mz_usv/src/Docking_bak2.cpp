...
#include <tf/tf.h>
#define CENTER 480
#define SleepTime1 1500000
#define SleepTime2 3000000
#define SleepTime3 2500000
#define AdSpeed 0.6
#define Low 400
class Docking
{
	...
	int rarea=0,barea=0,rcenterX,bcenterX,centerX,rbrate=10;
	float k_ad=1.0;
	...
	void running()
	{
		...
		while(ros::ok())
		{
			...
			if((rarea>=Low&&rarea<4000&&  rbrate>=5)||( rarea>=4000 &&rarea<=30000))//追踪:红色蓝色都比较小 或者红色比较大
			{
				//printf("Docking: %d\n",rcenterX);
				ROS_INFO("tracking");
				                ROS_INFO("red:%d,,,,blue:%d",rarea,barea);
			...
			else if(rarea>Low&&  rbrate<5 &&rarea<4000){//调整前进：红色比较小，蓝色比较大
			//	count++;
			//	if (count>0){
					count=0;
