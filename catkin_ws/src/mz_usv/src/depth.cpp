...
void altimeter_actionRun(void)
{
	...
	int readlen;
	ros::NodeHandle nh;
    ros::Publisher pub = nh.advertise<mz_usv::MS5837>("depth",4);
    mz_usv::MS5837 p;
    ros::Rate rate(50);
	while(1)
	{	
		readlen=read(altimeter_fd, altimeterbuf, 50);
		if(readlen>0)
		{
...
{	
	...
	//开启声学串口  9600 8 N 1 
    if ((altimeter_fd = open_port(altimeter_fd, 5)) < 0)
    {
        perror("open_port error"); 
        return 0;
    }
    if((i=set_opt(altimeter_fd,115200,8,'N',1))<0)
    {
